...
class Cam:
    ...
    def blit(self,b,window):
        lx=self.lights[self.steplights]
        if not lx.stop:
            lx.cacul(b)
...
class Black:
    def __init__(self,M=10**35,rcp=6,h=3*10**6):#rcp=5.5#M=10**35,h=3*10**6
        self.M=M
...
class Play(Movie):
    def __init__(self):
        self.b=Black()
        p=vector(7*self.b.rs,0*self.b.rs,0.1*self.b.rs)#10,0,0.5#7,0,0.1
        self.cam=Cam(p,200)
